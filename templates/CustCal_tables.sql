...
,stkbuyqty_ex   numeric(20,2)   --一级市场买入数量(如LOF、ETF申购、基金认购等)
,stkbuyamt_ex   numeric(20,2)   --其他买入金额*(不参与交易量统计,非交易量金额，如ETF申赎现金替代、转债转股资金、行权资金)
,stksaleqty_ex  numeric(20,2)   --一级市场卖出数量(如LOF、ETF赎回、基金赎回等)
...
,stkdebt_ch     numeric(20,2)   --借入数量变动
,stkloan        numeric(20,2)   --借出数量
,stkloan_ch     numeric(20,2)   --借出数量变动
...
,cjsr           numeric(20,2)   --出借利息收入，出借资券业务的收入        
,cjsr_ch        numeric(20,2)   --出借利息收入变动        
,jrzc           numeric(20,2)   --借入利息(费用)支出，借入资券业务的支出，含质押式融资融券               
,jrzc_ch        numeric(20,2)   --借入利息(费用)支出变动        
,sxf            numeric(20,2)   --手续费
,sxf_ch         numeric(20,2)   --手续费变动
,yhs            numeric(20,2)   --印花税
,yhs_ch         numeric(20,2)   --印花税变动 
,lxs            numeric(20,2)   --利息税
,lxs_ch         numeric(20,2)   --利息税变动
,jsxf           numeric(20,2)   --券商净佣金        
,jsxf_ch        numeric(20,2)   --券商净佣金变动
,ghf            numeric(20,2)   --过户费          
,ghf_ch         numeric(20,2)   --过户费变动
,qtfee          numeric(20,2)   --其他费          
...
go   
if exists(select * from sysobjects where xtype='U' and name='ETF_Sgsh')
   drop table ETF_Sgsh
go
create table ETF_Sgsh(
 serverid       char(1)            --节点编号
,bizdate        int                --系统日期
,sno            int                --流水日期
,orgid          char(4)            --机构号
,custid         bigint             --核算帐户
,ordersno       int                --委托序号
,busintype      int                --业务类型
,businname      varchar(30)        --业务类型名称
,market	        char(1)            --交易市场
,stkcode        varchar(20)        --ETF代码
,matchqty       bigint             --ETF份额数量
,matchamt       numeric(20,2)      --ETF份额金额
,xjtd           numeric(20,2)      --现金替代 
,stkcost        numeric(20,2)      --成份股
,tx_xjtd        numeric(20,2)      --已摊_现金替代 
,tx_stkcost     numeric(20,2)      --已摊_成份股
,tx_ct          int                --已摊次数
,lastsno        int                --最后摊销流水号(成份股)
,ref_date       int                --摊销日期,主要指现金替代，成份股一般为同一天
,ref_sno        int                --最后摊销流水号(现金替代) 
,status         char(1)            --0 未摊销 1在摊销 2已摊完
,remark         varchar(128)       --备注信息
constraint ETF_Sgsh_pk primary key nonclustered(sno, bizdate, serverid, custid)
)
go