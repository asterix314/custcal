...
,@action         varchar(3)      --动作类型--0B/0S买入/卖出     *二级市场买卖交易，一般会实际产生手续费
                                           --1B/1S申购/赎回，   *一级市场交易
...
,@syvalue_ch     numeric(20,2)   --*投资收益，卖出或划出时，按照卖出金额减去摊销成本记增，利息税，融资融券利息或费用时需要借用此字段
...
,@sxf            numeric(20,2)   --手续费
,@jsxf           numeric(20,2)   --净手续费
...
declare @stkbuyqty as numeric(20,2), @stkbuyamt as numeric(20,2), @stksaleqty as numeric(20,2), @stksaleamt as numeric(20,2), 
        @stkbuyqty_ex as numeric(20,2), @stkbuyamt_ex as numeric(20,2), @stksaleqty_ex as numeric(20,2), @stksaleamt_ex as numeric(20,2),
        @stkztgrqty as numeric(20,2), @stkztgramt as numeric(20,2), @stkztgcqty as numeric(20,2), @stkztgcamt as numeric(20,2), 
        @stkhgqty as numeric(20,2), @stkhlamt as numeric(20,2), @stkpgqty as numeric(20,2), @stkpgamt as numeric(20,2),
        @stkadjust as numeric(20,2), @stkpledge as numeric(20,2), @stkdebt as numeric(20,2), @stkloan as numeric(20,2),
        @cjsr as numeric(20,2), @jrzc as numeric(20,2), @lxs as numeric(20,2),
        @direction as int, @rowcount as int

 select @stkbuyqty=0, @stkbuyamt=0, @stksaleqty=0, @stksaleamt=0, 
   ...
   --动作类型--0B/0S买入/卖出     *二级市场买卖交易，一般会实际产生手续费
             --1B/1S申购/赎回，   *一级市场交易
             --ZR/ZC内部转入/转出 *资产不同形式资产的转换，比如ETF股票换基金，可转债转换为股票等
             --WR/WC外部转入/转出 *资产向我公司之外转出或者从外部转入进来
             --HG/PG红股红利/配股 
             ...
             --RR/RC证券融入/融出       
             --EB/ES ETF申购/赎回 
   if (@action in ('0B','1B','ZR','WR','HG','HL','PG','WR','ZYR','RR', 'EB'))
       begin
          select @direction='1'    
       end
    else if (@action in ('0S','1S','ZC','WC','ZYC','RC', 'ES'))
       begin
          select @direction='2'    
       end
    else   
   ...
   if (@direction='1' and @stkeffect<0)     
       begin
          select @msg='证券发生方向不符合(direction=1).'       
          raiserror(' %s', 12, 2, @msg) with SETERROR          
       end    
       
   if (@direction='2' and @stkeffect>0)     
       begin
          select @msg='证券发生方向不符合(direction=2).'       
          raiserror(' %s', 12, 2, @msg) with SETERROR          
       end                 
          
   if (@direction='1' and @fundeffect!=-(@matchamt+@sxf+@ghf+@yhs+@qtfee))     
       begin
          select @msg='资金发生数校验不平(direction=1).'       
          raiserror(' %s', 12, 2, @msg) with SETERROR          
       end

   if (@direction='2' and @fundeffect!=@matchamt-@sxf-@ghf-@yhs-@qtfee)    
       begin
   ...
   if (@action='0B')        --0B买入
    ...
    else if (@action='1B')  --1S认购或申购
       begin
          select @stkbuyqty_ex=@matchqty, @stkbuyamt_ex=@matchamt       
       end       
    else if (@action='1S')  --1S赎回
       begin
          select @stksaleqty_ex=@matchqty, @stksaleamt_ex=@matchamt       
       end       
   ...
   if (@direction='1' and @stkbuyamt+@stkbuyamt_ex+@stkztgramt+@stkpgamt!=@stkcost_ch+@aicost_ch) 
       begin
          select @msg='买入项累计资金和成本不符.'       
          raiserror(' %s', 12, 2, @msg) with SETERROR                  
       end

   if (@direction='2' and @stksaleamt+@stksaleamt_ex+@stkztgcamt+@stkhlamt+@stkcost_ch+@aicost_ch!=@syvalue_ch+@lxsr_ch) 
       begin
          select @msg='卖出项累计资金和成本收益不符.'       
          raiserror(' %s', 12, 2, @msg) with SETERROR                  
       end
...
begin try
   if (@direction='1')               --买入类，不需要校验持仓，如果无则需新增记录
       begin
          update stkasset_hs
             set stkbuyqty=stkbuyqty+@stkbuyqty, stkbuyamt=stkbuyamt+@stkbuyamt, 
                 stkbuyqty_ex=stkbuyqty_ex+@stkbuyqty_ex, stkbuyamt_ex=stkbuyamt_ex+@stkbuyamt_ex,
                 ...
                 stkdebt_ch=stkdebt_ch+@stkdebt, stkloan_ch=stkloan_ch+@stkloan,                 
                 ...
                 mktvalue=mktvalue+@stkcost_ch, aiamount=aiamount+@aicost_ch,
                 aicost=aicost+@aicost_ch, aicost_ch=aicost_ch+@aicost_ch,
                 cjsr=cjsr+@cjsr, cjsr_ch=cjsr_ch+@cjsr, 
                 jrzc=jrzc+@jrzc, jrzc_ch=jrzc_ch+@jrzc,                   
                 sxf=sxf+@sxf, sxf_ch=sxf_ch+@sxf,
       ...
       begin
          update stkasset_hs
             set stksaleqty=stksaleqty+@stksaleqty, stksaleamt=stksaleamt+@stksaleamt, 
                 stksaleqty_ex=stksaleqty_ex+@stksaleqty_ex, stksaleamt_ex=stksaleamt_ex+@stksaleamt_ex,
                 ...
                 qtfee=qtfee+@qtfee, qtfee_ch=qtfee_ch+@qtfee
           where stkcode=@stkcode and market=@market and fundid=@fundid and custid=@custid and orgid=@orgid and 
                 ltlx=@ltlx and stkqty+@stkeffect+stkdebt-stkloan>=0 and serverid=@serverid       
       end
           ...
           begin
              insert stkasset_hs
                     (serverid, orgid, custid, fundid, market, stkcode, ltlx, 
                      stkbuyqty, stkbuyamt, stksaleqty, stksaleamt, 
                      stkbuyqty_ex, stkbuyamt_ex, stksaleqty_ex, stksaleamt_ex, 
                      stkztgrqty, stkztgramt, stkztgcqty, stkztgcamt, 
                      stkhgqty, stkhlamt, stkpgqty, stkpgamt, 
                      stkqty, stkqty_ch, stkqty_tz, stkqty_tzje, 
                      stkpledge, stkdebt, stkdebt_ch, stkloan, stkloan_ch, 
                      stkadjust, stkadjust_ch, stkprice, bondintr, mktvalue, aiamount, 
                      stkcost, stkcost_ch, aicost, aicost_ch, 
                      syvalue, syvalue_ch, lxsr, lxsr_ch, 
                      gyvalue, gyvalue_ch, lxjt, lxjt_ch, 
                      cjsr, cjsr_ch, jrzc, jrzc_ch,  
                      sxf, sxf_ch, jsxf, jsxf_ch, 
                      yhs, yhs_ch, lxs, lxs_ch, 
               ...
               select @serverid, @orgid, @custid, @fundid, @market, @stkcode, @ltlx, 
                      ...
                      @stkztgrqty, @stkztgramt, @stkztgcqty, @stkztgcamt, 
                      @stkhgqty, @stkhlamt, @stkpgqty, @stkpgamt, 
                      stkqty=@stkeffect, stkqty_ch=@stkeffect, stkqty_tz=0, stkqty_tzje=0, 
                      stkpledge=@stkpledge, @stkdebt, stkdebt_ch=@stkdebt, @stkloan, stkloan_ch=@stkloan, 
                      stkadjust=@stkadjust, stkadjust_ch=@stkadjust, stkprice=0, bondintr=0, mktvalue=@stkcost_ch, @aiamount, 
                      stkcost=@stkcost_ch, @stkcost_ch, aicost=@aicost_ch, @aicost_ch, 
                      syvalue=@syvalue_ch, @syvalue_ch, lxsr=@lxsr_ch, @lxsr_ch, 
                      gyvalue=0, gyvalue_ch=0, lxjt=0, lxjt_ch=0, 
                      @cjsr, cjsr_ch=@cjsr, @jrzc, jrzc_ch=@jrzc, 
                      @sxf, sxf_ch=@sxf, @jsxf, jsxf_ch=@jsxf, 
                      @yhs, yhs_ch=@yhs, @lxs, lxs_ch=@lxs, 
...
begin catch
   select @msg='写资券资产记录错误:('+LEFT(ERROR_MESSAGE(),32)+')'
   return -1
end catch
go
