/********************************************
    ...
    流通类型：  01
********************************************/
if exists (select * from sysobjects where type='P' and name='sp_Cust_XG_XGZQ')
    drop proc sp_Cust_XG_XGZQ
go
/*
select * from logasset where sno=162539 and bizdate=20150115
select * from logasset where sno=145837 and bizdate=20150115

declare @msg as varchar(128)
exec sp_Cust_XG_XGZQ 20150115, 145837, 160100030966, @msg output
select @msg

select * from stkasset_hs where custid=160100030966 and stkcode='300418'
*/
CREATE proc sp_Cust_XG_XGZQ(
 @serverid  char(1)      
,@bizdate   int          
,@sno       int
,@custid    bigint    
...
declare @rowcount as int,@ref_stkcode as varchar(10), @expense as numeric(20,2), @ret as int
 ...
 select @orgid=orgid, @fundid=fundid, @moneytype=moneytype, @digestid=digestid, @market=market, @stkcode=stkcode,
  ...
  where sno=@sno and bizdate=@bizdate and serverid=@serverid and digestid=220027
...
begin try
   if (@sett_status is null)
    ...
    select @ref_stkcode=linkstk from stktrd where stkcode=@stkcode and market=@market
    if (isnull(@ref_stkcode,'')='')
       select @ref_stkcode=@stkcode


begin tran
    exec @ret=nb_Cust_Stkasset_Commit
              @serverid=@serverid, @orgid=@orgid, @custid=@custid, @fundid=@fundid, @moneytype=@moneytype,
              @bankcode=@bankcode, @action='0B', @market=@market, @stkcode=@ref_stkcode, @ltlx='01',
              @matchqty=@matchqty, @matchamt=@matchamt, @matchamt_ex=0, @aiamount=0, @fundeffect=@fundeffect,
              @stkeffect=@stkeffect, @stkcost_ch=@matchamt, @syvalue_ch=0, @aicost_ch=0, @lxsr_ch=0,
              @fee=0, @jsxf=0, @yhs=0, @ghf=0,
