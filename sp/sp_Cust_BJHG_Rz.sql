...
if exists (select * from sysobjects where type='P' and name='sp_Cust_BJHG_Rqgh')
    drop proc sp_Cust_BJHG_Rqgh
go
/*
select * from logasset where digestid=221033
declare @msg as varchar(128)
exec sp_Cust_BJHG_Rqgh 20150105, 1, 100, @msg output
select @msg
*/
CREATE proc sp_Cust_BJHG_Rqgh( @serverid  int
,@bizdate   int
,@sno       int
,@custid    bigint
...
declare @rowcount as int, @expense as numeric(20,2), @ret as int
 ...
 select @orgid=orgid, @fundid=fundid, @moneytype=moneytype, @digestid=digestid, @market=market,
        @stkcode=stkcode, @bankcode=bankcode, @fundeffect=fundeffect, @stkeffect=stkeffect, @matchqty=matchqty,
	@matchamt=matchamt, @fee_sxf=fee_sxf, @fee_jsxf=fee_jsxf, @fee_ghf=fee_ghf, @fee_yhs=fee_yhs,
	@feefront=feefront, @sett_status=sett_status, @sett_remark=sett_remark,
	@expense=fee_sxf + fee_ghs + fee_yhs + feefront
   from logasset with (nolock, index=index_of_logasset_pk)
  where sno=@sno and bizdate=@bizdate and serverid=@serverid and digestid=221033
