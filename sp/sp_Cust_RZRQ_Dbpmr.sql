/********************************************
    ...
    流通类型：  00
********************************************/
if exists (select * from sysobjects where type='P' and name='sp_Cust_RZRQ_Dbpmr')
    drop proc sp_Cust_RZRQ_Dbpmr
go
/*
exec sp_Cust_RZRQ_Dbpmr '1',20150202, 63435, 129500044309, @msg output

*/
CREATE proc sp_Cust_RZRQ_Dbpmr(
 @serverid  int
,@bizdate   int
,@sno       int
,@custid    bigint
...
declare @rowcount as int, @expense as numeric(20,2), @ret as int
 ...
 select @orgid=orgid, @fundid=fundid, @moneytype=moneytype, @digestid=digestid, @market=market, @stkcode=stkcode,
  ...
  where sno=@sno and bizdate=@bizdate and serverid=@serverid and digestid=550001
