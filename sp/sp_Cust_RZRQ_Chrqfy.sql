/********************************************
    ...
    流通类型：  07
********************************************/
if exists (select * from sysobjects where type='P' and name='sp_Cust_RZRQ_Chrqfy')
    drop proc sp_Cust_RZRQ_Chrqfy
go
/*
declare @msg as varchar(128)
exec sp_Cust_RZRQ_Chrqfy  20150105, 1, 100, @msg output
select @msg
*/
CREATE proc sp_Cust_RZRQ_Chrqfy(
 @serverid  int
,@bizdate   int
,@sno       int
,@custid    bigint
...
declare @rowcount as int, @expense as numeric(20,2), @ret as int
 ...
 select @orgid=orgid, @fundid=fundid, @moneytype=moneytype, @digestid=digestid, @market=market, @stkcode=stkcode,
  ...
  where sno=@sno and bizdate=@bizdate and serverid=@serverid and digestid=552003
...
begin tran
        ...
        using (
                select serverid, orgid, custid, fundid, market, stkcode, '07' as ltlx, moneytype, bankcode,
                       fundeffect
                  from logasset_hs
                 where sno=@sno and bizdate=@bizdate and serverid=@serverid and digestid=552003
        ) as ord
        ...
        when matched then
                update set rqlx = rqlx + ord.fundeffect, rqlx_ch = rqlx_ch + ord.fundeffect
        when not matched then
                insert (serverid, orgid, custid, fundid, market, stkcode, ltlx, rqlx, rqlx_ch)  
