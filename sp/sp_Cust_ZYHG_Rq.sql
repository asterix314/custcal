/********************************************
    ...
    流通类型：  07
********************************************/
if exists (select * from sysobjects where type='P' and name='sp_Cust_ZYHG_Rq')
    drop proc sp_Cust_ZYHG_Rq
go
/*
declare @msg as varchar(128)
exec sp_Cust_ZYHG_Rq 20150105, 1, 100, @msg output
select @msg
*/
CREATE proc sp_Cust_ZYHG_Rq(
 @serverid  char(1)      
,@bizdate   int          
,@sno       int
,@custid    bigint    
...
declare @rowcount as int, @expense as numeric(20,2), @ret as int
 ...
 select @orgid=orgid, @fundid=fundid, @moneytype=moneytype, @digestid=digestid, @market=market, @stkcode=stkcode,
  ...
  where sno=@sno and bizdate=@bizdate and serverid=@serverid and digestid=220003
