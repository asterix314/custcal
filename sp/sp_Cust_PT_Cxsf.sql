/********************************************
    ...
    流通类型：  00
********************************************/
if exists (select * from sysobjects where type='P' and name='sp_Cust_PT_Cxsf')
    drop proc sp_Cust_PT_Cxsf
go
/*
select * from logasset where digestid=222006
declare @msg as varchar(128)
exec sp_Cust_PT_Cxsf  20150105, 1, 100, @msg output
select @msg
*/
CREATE proc sp_Cust_PT_Cxsf(
 @serverid  int
,@bizdate   int
,@sno       int
,@custid    bigint
