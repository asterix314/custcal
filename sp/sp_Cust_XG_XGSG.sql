/********************************************
    ...
    流通类型：  03
********************************************/
if exists (select * from sysobjects where type='P' and name='sp_Cust_XG_XGSG')
    drop proc sp_Cust_XG_XGSG
go
/*
...
declare @rowcount as int, @expense as numeric(20,2), @ret as int
 ...
 select @orgid=orgid, @fundid=fundid, @moneytype=moneytype, @digestid=digestid, @market=market, @stkcode=stkcode,
  ...
  where sno=@sno and bizdate=@bizdate and serverid=@serverid and digestid=220023
...
begin tran
    exec @ret=nb_Cust_Stkasset_Commit
              ...
              @bankcode=@bankcode, @action='0B', @market=@market, @stkcode=@stkcode, @ltlx='03',
              ...
              @fee=@fee_sxf + @fee_ghf + @feefront, @jsxf=@fee_jsxf, @yhs=@fee_yhs, @ghf=@fee_ghf,
