/********************************************
    ...
    流通类型：  00
********************************************/
if exists (select * from sysobjects where type='P' and name='sp_Cust_KFJJ_Sg')
    drop proc sp_Cust_KFJJ_Sg
go
/*
select * from logasset where digestid=220049
declare @msg as varchar(128)
exec sp_Cust_KFJJ_Sg 20150105, 1, 100, @msg output
select @msg
*/
CREATE proc sp_Cust_KFJJ_Sg(
 @serverid  char(1)
,@bizdate   int
,@sno       int
,@custid    bigint,
...
begin tran
    exec @ret=nb_Cust_Stkasset_Commit
              ...
              @bankcode=@bankcode, @action='EB', @market=@market, @stkcode=@stkcode, @ltlx='00',
