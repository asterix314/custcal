...
if exists (select * from sysobjects where type='P' and name='sp_Cust_XG_SGFK')
    drop proc sp_Cust_XG_SGFK
go
/*
select * from logasset where digestid=221024 and sett_status='4' and custid=129500002198  order by bizdate,sno
declare @msg as varchar(128)
exec sp_Cust_XG_SGFK 20150320, 87547, 129500002198, @msg output
select @msg

select * from stkasset_hs where custid=129500002198 and stkcode='160221'
*/
CREATE proc sp_Cust_XG_SGFK(
 @serverid  char(1)      
,@bizdate   int          
,@sno       int
,@custid    bigint    
...
declare @rowcount as int, @expense as numeric(20,2), @ret as int
 ...
 select @orgid=orgid, @fundid=fundid, @moneytype=moneytype, @digestid=digestid, @market=market, @stkcode=stkcode,
  ...
  where sno=@sno and bizdate=@bizdate and serverid=@serverid and digestid=221024
...
begin try
   if (@sett_status is null)
    ...
    if (@fundeffect=0 and @stkeffect=0)
       begin
          select @msg='新股申购返款数量金额为0无须处理'
          update logasset_hs
             set sett_status=3, sett_remark=@msg
           where sno=@sno and bizdate=@bizdate and custid=@custid and serverid=@serverid
    ...
    if (@matchamt-@fundeffect!=0 or @expense != 0)
       begin
...
begin tran
    exec @ret=nb_Cust_Stkasset_Commit
              ...
              @bankcode=@bankcode, @action='OS', @market=@market, @stkcode=@stkcode, @ltlx='03',
              ...
              @fee=0, @jsxf=0, @yhs=0, @ghf=0, @qtfee=0, @lxs=0, @blje=0, @blxx='', @msg=@msg output
