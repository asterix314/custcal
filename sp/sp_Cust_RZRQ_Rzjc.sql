/********************************************
    ...
    流通类型：  -
********************************************/
if exists (select * from sysobjects where type='P' and name='sp_Cust_RZRQ_Rzjc')
    drop proc sp_Cust_RZRQ_Rzjc
go
/*
declare @msg as varchar(128)
exec sp_Cust_RZRQ_Rzjc  20150105, 1, 100, @msg output
select @msg
*/
CREATE proc sp_Cust_RZRQ_Rzjc(
 @serverid  int
,@bizdate   int
,@sno       int
,@custid    bigint
...
declare @rowcount as int, @expense as numeric(20,2), @ret as int
 ...
 select @orgid=orgid, @fundid=fundid, @moneytype=moneytype, @digestid=digestid, @market=market, @stkcode=stkcode,
  ...
  where sno=@sno and bizdate=@bizdate and serverid=@serverid and digestid=553002
...
begin tran
        ...
        when matched then
                update set fundbal = fundbal + ord.fundeffect, fundbal_ch = ord.fundeffect,
                           fundloan = fundload - ord.fundeffect, fundloan_ch = fundloan_ch - ord.fundeffect
        when not matched then
                insert (serverid, orgid, custid, bankcode, fundid, moneytype, fundbal, fundbal_ch, fundloan, fundloan_ch)  
