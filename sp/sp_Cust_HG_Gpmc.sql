/********************************************
    ...
    流通类型：  00
********************************************/
if exists (select * from sysobjects where type='P' and name='sp_Cust_HG_Gpmc')
    drop proc sp_Cust_HG_Gpmc
go
/*
...
exec sp_Cust_HG_Gpmc @serverid=1, @bizdate=20150105, @sno=148741, @custid=129500065129, @msg=@msg output
select @msg

select * from stkasset_hs
*/
CREATE proc sp_Cust_HG_Gpmc(
 @serverid  char(1)      
,@bizdate   int          
,@sno       int
,@custid    bigint    
...
declare @stkcost as numeric(20,2), @rowcount as int, @expense as numeric(20,2), @ret as int
 ...
 select @orgid=orgid, @fundid=fundid, @moneytype=moneytype, @digestid=digestid, @market=market, @stkcode=stkcode,
  ...
  where sno=@sno and bizdate=@bizdate and serverid=@serverid and digestid=220095
